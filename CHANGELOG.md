...
## v0.15.6
- **FIXED (OOM blocker): removed `sing-box version` last-resort fallback** in `get_singbox_version_display()`. The fallback spawned a full Go runtime (+20-30 MB RSS) even after the state file/opkg/apk sources were added. On 256 MB routers this could still trigger OOM-killer in Maintenance/Daily Report/Support Bundle hot paths. Now falls through to `"unknown"` without any binary spawn.
- **FIXED: broken `sed` expression in apk version parser** — `s/ *//)` had an unmatched `)` causing `sed: unknown option to 's'` at runtime in the apk branch. Fixed to `s/[[:space:]].*//'` which correctly strips arch/metadata suffix from `apk list --installed` output (`sing-box-extended-1.13.12 aarch64 {...}` → `1.13.12`).
- **FIXED (Plus): subscription server count showed `0`** — `get_subscription_server_count` now uses Clash `/proxies` as primary source (filters out DIRECT/BLOCK/dns-out), with a file-based fallback that correctly uses `(.servers // {}) | length` for Plus map format and the previous `.outbounds[]` path for other variants.
- **FIXED (Plus): wrong subscription cache path** — `get_subscription_cache_path` was reading from `subscription-links/<sec>.json` (empty stubs) instead of `section-cache/<sec>.json` (real node data). Aligned with actual Plus cache layout.
- **FIXED (Plus): active outbound display showed `subscription` instead of real node** — `get_active_proxy_display` for Plus now shows `subscription · urltest → <node> (N)` / `subscription · selector → <node> (N)`. Non-Plus path preserved.
- **FIXED (Plus): Proxy Mode card button highlight wrong** — mode in Plus is determined by `urltest_enabled` UCI field. Correct button now gets ✓. No-op guard added in `ask_switch_mode_*`: skips reload if target mode already active.
- **FIXED (Plus): Status / Daily Report mode label** — `_sec_mode_disp` for Plus subscription sections now shows `Subscription · URLTest` / `Subscription · Selector` instead of a bare single value.
- **FIXED (OOM/Plus): `_plus_sub_metadata` reads section cache instead of spawning Plus CLI** — new helper reads `.subscriptionMetadata` from `section-cache/<sec>.json`. CLI fallback kept internally but removed from Outbounds and Daily Report hot paths, eliminating subprocess spawn on card open.
- **FIXED: Server Instances card garbled on mobile** — renderer changed from `printf '%b'` (interpreted all backslash escapes including `\t`, `\xNN`, corrupting UCI values) to `awk '{gsub(/\\n/,"\n")}'` which expands only literal `\n` to real newlines. Confirmed root cause of the character-by-character layout breakage. Tailscale entry enhanced: new helpers `_si_safe_name` / `_si_ts_state_dir` / `_si_ts_registered`; shows `🔗 connected` / `🔌 not registered`, hostname with default, control URL (Headscale support), exit-node/accept-routes, per-section state path; `pgrep` moved outside the loop (one call per card).
