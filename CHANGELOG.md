...
- REFACTORED: All syslog and Telegram alert messages rewritten for readability.
         Syslog: internal variable dumps (RECOVERY_MODE, FAST/POLL, method=) replaced
         with human-readable sentences. Cache lines renamed [Core]. Health summary
         reformatted as single [Health] line. Watchdog IPC messages now describe
         actions, not internal mechanics.
         Telegram alerts: sing-box stop/recover, SOCKS down/recover, proxy switch,
         TG connectivity — all rewritten to focus on user impact, removing
         technical terms (tier1, control plane, Scope:) in favour of plain English.
