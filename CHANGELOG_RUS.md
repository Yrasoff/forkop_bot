...
## v0.15.6
- **ИСПРАВЛЕНО (OOM-блокер): убран last-resort fallback `sing-box version`** в `get_singbox_version_display()`. Fallback запускал полный Go-рантайм (+20-30 MB RSS) даже после добавления state file/opkg/apk источников. На роутерах с 256 MB это всё равно могло вызвать OOM-killer в hot path Maintenance/Daily Report/Support Bundle. Теперь при отсутствии версии возвращается `"unknown"` без спавна бинаря.
- **ИСПРАВЛЕНО: сломанный `sed` в apk-парсере версии** — `s/ *//)` содержал непарную `)` и давал `sed: unknown option to 's'` в runtime. Исправлено на `s/[[:space:]].*//'` — корректно обрезает суффикс архитектуры из вывода `apk list --installed` (`sing-box-extended-1.13.12 aarch64 {...}` → `1.13.12`).
- **ИСПРАВЛЕНО (Plus): счётчик серверов подписки показывал `0`** — `get_subscription_server_count` теперь использует Clash `/proxies` как первичный источник (фильтрует DIRECT/BLOCK/dns-out), с file-fallback который корректно читает `(.servers // {}) | length` для Plus map-формата.
- **ИСПРАВЛЕНО (Plus): неверный путь кэша подписки** — `get_subscription_cache_path` читал из `subscription-links/<sec>.json` (пустые стабы) вместо `section-cache/<sec>.json` (реальные ноды). Выровнено с реальной раскладкой кэша Plus.
- **ИСПРАВЛЕНО (Plus): активный outbound показывал `subscription` вместо реального сервера** — `get_active_proxy_display` для Plus теперь показывает `subscription · urltest → <node> (N)` / `subscription · selector → <node> (N)`. Не-Plus путь сохранён.
- **ИСПРАВЛЕНО (Plus): неверная подсветка кнопки режима в Proxy Mode card** — режим для Plus определяется полем UCI `urltest_enabled`. Правильная кнопка теперь получает ✓. Добавлен no-op guard в `ask_switch_mode_*`: если режим уже активен — пропускает reload.
- **ИСПРАВЛЕНО (Plus): лейбл режима в Status / Daily Report** — `_sec_mode_disp` для Plus subscription-секций теперь показывает `Subscription · URLTest` / `Subscription · Selector`.
- **ИСПРАВЛЕНО (OOM/Plus): `_plus_sub_metadata` читает section cache вместо спавна Plus CLI** — новый хелпер читает `.subscriptionMetadata` из `section-cache/<sec>.json`. CLI-fallback сохранён внутри, но убран из hot path Outbounds и Daily Report.
- **ИСПРАВЛЕНО: каша в карточке Server Instances на мобильном** — рендерер заменён с `printf '%b'` на `awk '{gsub(/\\n/,"\n")}'` который разворачивает только буквальные `\n`. Подтверждённый корень посимвольного развала. Tailscale-запись улучшена: новые хелперы `_si_safe_name` / `_si_ts_state_dir` / `_si_ts_registered`; `🔗 connected` / `🔌 not registered`, hostname, control URL (Headscale), exit-node/accept-routes, per-section стейт; `pgrep` вынесен из цикла.
